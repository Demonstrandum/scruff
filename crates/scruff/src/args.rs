...
pub struct GlobalConfigArgs {
    ...
    // If a user specifies `scruff check --isolated --config=scruff.toml`,
...
///
/// ```sh
/// scruff check --config "path/to/scruff.toml" --config "extend-select=['E501', 'F841']" --config "lint.per-file-ignores = {'some_file.py' = ['F841']}"
/// ```
#[derive(Clone, Debug)]
