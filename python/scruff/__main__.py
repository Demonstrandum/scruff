...
def find_scruff_bin() -> str:
    ...
    scripts_path = os.path.join(sysconfig.get_path("scripts"), scruff_exe)
    ...
    user_path = os.path.join(
        sysconfig.get_path("scripts", scheme=user_scheme), scruff_exe
    )
    ...
    target_path = os.path.join(pkg_root, "bin", scruff_exe)
    if os.path.isfile(target_path):
        return target_path

    # Search for pip-specific build environments.
    #
    # Expect to find scruff in <prefix>/pip-build-env-<rand>/overlay/bin/scruff
    ...
    if len(paths) >= 2:
        ...
        if (
            ...
            # The overlay must contain the scruff binary.
            candidate = os.path.join(paths[0], scruff_exe)
...
if __name__ == "__main__":
    scruff = find_scruff_bin()
    if sys.platform == "win32":
        import subprocess

        completed_process = subprocess.run([scruff, *sys.argv[1:]])
        sys.exit(completed_process.returncode)
    else:
        os.execvp(scruff, [scruff, *sys.argv[1:]])
